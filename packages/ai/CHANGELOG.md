...
- Fixed concurrent Cursor callers with different credentials shutting down one another, concurrent first use creating duplicate runtimes, generic Connect errors hiding structured diagnostic details, unbounded host-identity commands, HTTP/2 sessions leaking when connection completed after shutdown, cross-provider tool IDs violating Cursor's charset, colliding after normalization, or repeating across turns, public stop-sequence/tool-choice options being dropped, authoritative final text corrections being lost by the leaked-thinking wrapper, later user turns reusing stale routing conversations, and generic GPT routing forcing a 272K context value instead of the selected catalog context.
